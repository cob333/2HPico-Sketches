...
static int DelayLineSamplesAlloc(float sr, float i_pitch_mod, int n);
...
int ReverbSc::Init(float sr)
{
    ...
    for(i = 0; i < 8; i++)
    {
        int needed = DelayLineSamplesAlloc(sr, 1, i);
        if(n_samples + needed > DSY_REVERBSC_MAX_SIZE)
            return 1;
        delay_lines_[i].buf = (aux_) + n_samples;
...
static int DelayLineSamplesAlloc(float sr, float i_pitch_mod, int n)
{
    return DelayLineMaxSamples(sr, i_pitch_mod, n);
}
